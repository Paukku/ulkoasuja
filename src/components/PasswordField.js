...
class PasswordField extends Component {
  ...
  validatePasswordStrong = value => {
    ...
    if (value.length <= this.thresholdLength) throw new Error("Salasana on liian lyhyt. Salasanan tulee olla vähintään 7 merkkiä pitkä");

    // ensure password is strong enough using the zxcvbn library
    if (zxcvbn(value).score < this.minStrength) throw new Error("Salasana on heikko. Sen tulisi sisältää pieniä ja isoja merkkejä sekä numeroita");
  };
